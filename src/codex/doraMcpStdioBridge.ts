/**
 * Dora MCP STDIO Bridge
 *
 * Minimal STDIO MCP server exposing a single tool `change_title`.
 * On invocation it forwards the tool call to an existing Dora HTTP MCP server
...
async function main() {
  ...
  if (!baseUrl) {
    ...
    process.stderr.write(
      '[dora-mcp] Missing target URL. Set HAPPY_HTTP_MCP_URL or pass --url <http://127.0.0.1:PORT>\n'
    );
    process.exit(2);
  }
  ...
  async function ensureHttpClient(): Promise<Client> {
    ...
    const client = new Client(
      { name: 'dora-stdio-bridge', version: '1.0.0' },
      { capabilities: { tools: {} } }
    );
  ...
  const server = new McpServer({
    name: 'Dora MCP Bridge',
    version: '1.0.0',
    description: 'STDIO bridge forwarding to Dora HTTP MCP',
  });
...
main().catch((err) => {
  try {
    process.stderr.write(`[dora-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
  } finally {
    process.exit(1);
  }
});
