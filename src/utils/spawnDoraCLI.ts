/**
 * Cross-platform Dora CLI spawning utility
 * 
 * ## Background
 * 
 ...
 * with no warning output when using Dora.
 * 
 * ## The Wrapper Strategy
 * 
 * We created a wrapper script `bin/dora.mjs` with a shebang `#!/usr/bin/env node`.
 * This allows direct execution on Unix systems and NPM automatically generates 
 * Windows-specific wrapper scripts (`dora.cmd` and `dora.ps1`) when it sees 
 ...
 * 
 * **Unix/Linux/macOS:**
 * 1. User runs `dora` command
 * 2. Shell directly executes `bin/dora.mjs` (shebang: `#!/usr/bin/env node`)
 * 3. `bin/dora.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 * 
 * **Windows:**
 * 1. User runs `dora` command  
 * 2. NPM wrapper (`dora.cmd`) calls `node bin/dora.mjs`
 * 3. `bin/dora.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 * 
 * ## The Spawning Problem
 * 
 * When our code needs to spawn Dora cli as a subprocess (for daemon processes), 
 * we were trying to execute `bin/dora.mjs` directly. This fails on Windows 
...
/**
 * Spawn the Dora CLI with the given arguments in a cross-platform way.
 * 
 * This function bypasses the wrapper script (bin/dora.mjs) and spawns the 
 * actual CLI entrypoint (dist/index.mjs) directly with Node.js, ensuring
 * compatibility across all platforms including Windows.
 * 
 * @param args - Arguments to pass to the Dora CLI
...
export function spawnHappyCLI(args: string[], options: SpawnOptions = {}): ChildProcess {
  ...
  // bypassing the 'dora' wrapper that would normally be found in the shell's PATH.
  // However, we log it as 'dora' here because other engineers are typically looking
  // for when "dora" was started and don't care about the underlying node process
  // details and flags we use to achieve the same result.
  const fullCommand = `dora ${args.join(' ')}`;
  logger.debug(`[SPAWN DORA CLI] Spawning: ${fullCommand} in ${directory}`);
  ...
  if (!existsSync(entrypoint)) {
    ...
    logger.debug(`[SPAWN DORA CLI] ${errorMessage}`);
