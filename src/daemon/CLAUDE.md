# Dora CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages Dora sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Daemon Lifecycle

### Starting the Daemon

Command: `dora daemon start`
...
4. `startDaemon()` performs startup:
   ...
   - RPC registration: exposes `spawn-dora-session`, `stop-session`, `requestShutdown` handlers
...
The daemon detects when `npm upgrade dora-coder` occurs:
...
1. Backend forwards RPC `spawn-dora-session` to daemon via WebSocket
2. `ApiMachineClient` invokes `spawnSession()` handler
3. `spawnSession()`:
   - Creates directory if needed
   - Spawns detached Dora process with `--dora-starting-mode remote --started-by daemon`
...
User runs `dora` directly:
1. CLI auto-starts daemon if configured
2. Dora process calls `notifyDaemonSessionStarted()` 
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'dora directly...'`
...
`dora doctor` uses `ps aux | grep` to find all Dora processes:
- Production: matches `dora.mjs`, `dora-coder`, `dist/index.mjs`
...
- Server to Daemon: rpc-request (spawn-dora-session, stop-session, requestShutdown)
