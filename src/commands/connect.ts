 ...
 * - connect codex: Store OpenAI API key in Dora cloud
 * - connect claude: Store Anthropic API key in Dora cloud
 * - connect gemini: Store Gemini API key in Dora cloud
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('dora connect')} - Connect AI vendor API keys to Dora cloud

${chalk.bold('Usage:')}
  dora connect codex        Store your Codex API key in Dora cloud
  dora connect claude       Store your Anthropic API key in Dora cloud
  dora connect gemini       Store your Gemini API key in Dora cloud
  dora connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in Dora cloud. This enables you to use these services through Dora
...
${chalk.bold('Notes:')} 
  • You must be authenticated with Dora first (run 'dora auth login')
  • API keys are encrypted and stored securely in Dora cloud
  • You can manage your stored keys at app.dora.engineering
`);
}

async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to Dora cloud\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with Dora'));
        console.log(chalk.gray('  Please run "dora auth login" first'));
        process.exit(1);
    }
