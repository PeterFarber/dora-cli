...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('dora auth')} - Authentication management

${chalk.bold('Usage:')}
  dora auth login [--force]    Authenticate with Dora
  dora auth logout             Remove authentication and machine data  
  dora auth status             Show authentication status
  dora auth show-backup        Display backup key for mobile/web clients
  dora auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'dora auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  console.log(chalk.blue('This will log you out of Dora'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use Dora again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      // Remove entire dora directory (as current logout does)
      if (existsSync(happyDir)) {
        rmSync(happyDir, { recursive: true, force: true });
      }

      console.log(chalk.green('✓ Successfully logged out'));
      console.log(chalk.gray('  Run "dora auth login" to authenticate again'));
    } catch (error) {
...
//     console.log(chalk.gray('Run "dora auth login" to authenticate first'));
//     return;
//   }
...
//   console.log(chalk.gray('• In Dora mobile app: Go to restore/link device and enter this key'));
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "dora auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "dora auth login --force" to fix this'));
  }
