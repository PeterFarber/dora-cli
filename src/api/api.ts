...
export class ApiClient {
  ...
  async getOrCreateSession(opts: {
    ...
    if (this.credential.encryption.type === 'dataKey') {
      ...
      // const contentDataKey = await deriveKey(this.secret, 'Dora EnCoder', ['content']);
  ...
  async getOrCreateMachine(opts: {
    ...
    if (response.status !== 200) {
      ...
      console.log(chalk.yellow(`[API] Failed to create machine: ${response.statusText}, most likely you have re-authenticated, but you still have a machine associated with the old account. Now we are trying to re-associate the machine with the new account. That is not allowed. Please run 'dora doctor clean' to clean up your dora state, and try your original command again. Please create an issue on github if this is causing you problems. We apologize for the inconvenience.`));
      process.exit(1);
    }
